...
const success = (position) => {
    ...
    .then((data) => {
        let a = new Intl.DateTimeFormat("en-US",{day: "2-digit", month: "short", year: "numeric"});
        let d = new Date(data.dt * 1000);
        let t = new Intl.DateTimeFormat("en-US",{hour12: "2-digit", minute: "2-digit", second: "2-digit"});
        ...
        console.log(data);
        let myString = `<h2><i class="fa-solid fa-location-dot"></i> ${data.name}</h2>
        <div class="box_1">
        <img src="http://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png" alt="Weather icon"> <span>${data.weather[0].main}</span>
        <h2 class="tmp"><i class="fa-solid fa-temperature-half"></i> ${data.main.temp}°C</h2>
    ...
    .catch(err => alert("Please input a proper city name !!!..."));
});
